...
function render(contacts) {
  contactsSection.innerHTML = contacts.map(renderContact).join("");
  loadCities(contacts);
}
...
function loadCities(contacts) {
  ...
  const uniqueArr = cities.filter(function (city, i, ar) {
    return ar.indexOf(city) === i;
  });

  const options = uniqueArr.map((city) => {
  ...
  options.unshift('<option value="0">-- Select a city --</option>');
  filterOptions.innerHTML = options.join("");
}
